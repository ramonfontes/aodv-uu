...
/* NS_PORT: Log filename is prefix + IP address + suffix */
    ...
    if (log_to_file) {
        if ((log_file_fd = open(AODV_LOG_PATH, O_RDWR | O_CREAT | O_TRUNC,
    ...
    if (rt_log_interval) {
        if ((log_rt_fd = open(AODV_RT_LOG_PATH, O_RDWR | O_CREAT | O_TRUNC,
                              S_IROTH | S_IWUSR | S_IRUSR | S_IRGRP)) < 0) {
...
/* NS_PORT: Include IP address in log */
    ...
    if (DEV_NR(NS_DEV_NR).enabled == 1) {
        len +=
            sprintf(log_buf + len, "%s: ", ip_to_str(DEV_NR(NS_DEV_NR).ipaddr));
    }
#endif /* NS_PORT */
    ...
    len +=
        sprintf(log_buf + len, "%02d:%02d:%02d.%03ld %s: %s", time->tm_hour,
                time->tm_min, time->tm_sec, now.tv_usec / 1000, function, msg);
            ...
            if (list_empty(&rt->precursors))
                len +=
                    sprintf(rt_buf + len,
                            ...
                            ip_to_str(rt->dest_addr), ip_to_str(rt->next_hop),
                            rt->hcnt, state_to_str(rt->state), seqno_str,
                            (rt->hcnt == 255)
                                ? 0
                ...
                len += sprintf(
                    ...
                    ip_to_str(rt->dest_addr), ip_to_str(rt->next_hop), rt->hcnt,
                    ...
                    rt_flags_to_str(rt->flags),
                    if_indextoname(rt->ifindex, ifname),
                    ip_to_str(((precursor_t *)rt->precursors.next)->neighbor));
