...
rt_table_t *NS_CLASS rt_table_insert(struct in_addr dest_addr,
                                     struct in_addr next, u_int8_t hops,
                                     ...
                                     unsigned int ifindex)
{
    ...
    //#ifdef NS_PORT
    DEBUG(LOG_INFO, 0, "New timer for %s, life=%d", ip_to_str(rt->dest_addr),
          life);
                                             ...
                                             rt_table_t *rev_rt)
{
    ...
    if (fwd_rt && fwd_rt->state == VALID) {
        ...
        if (llfeedback || fwd_rt->flags & RT_INET_DEST || fwd_rt->hcnt != 1 ||
            fwd_rt->hello_timer.used)
    ...
    for (i = 0; i < RT_TABLESIZE; i++) {
        ...
        list_foreach(pos, &rt_tbl.tbl[i])
        {
            ...
            if (rt->flags & RT_INET_DEST && rt->state == VALID) {
                rt_table_update(rt, gw->dest_addr, gw->hcnt, 0, life, VALID,
                                rt->flags);
                n++;
            }
        }
...
int NS_CLASS rt_table_invalidate(rt_table_t *rt)
{
            ...
            list_foreach(pos, &rt_tbl.tbl[i])
            {
                ...
                if (rt2->state == VALID && (rt2->flags & RT_INET_DEST) &&
                    ...
                    if (0) {
                        ...
                        rt_table_update(
                            rt2, gw->dest_addr, gw->hcnt, 0,
                            timeval_diff(&rt->rt_timer.timeout, &now), VALID,
                            rt2->flags);
                    } else {
    ...
    if (rt->flags & RT_REPAIR) {
        ...
        DEBUG(LOG_DEBUG, 0, "%s removed in %u msecs", ip_to_str(rt->dest_addr),
              DELETE_PERIOD);
    }
...
void NS_CLASS precursor_add(rt_table_t *rt, struct in_addr addr)
{
    ...
    DEBUG(LOG_INFO, 0, "Adding precursor %s to rte %s", ip_to_str(addr),
          ip_to_str(rt->dest_addr));
