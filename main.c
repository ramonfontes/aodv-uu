...
static void usage(int status)
{
    ...
    printf(
        "\nUsage: %s [-dghjlouwxLDRV] [-i if0,if1,..] [-r N] [-n N] [-q "
        "THR]\n\n"
        ...
        "(experimental).\n"
        "-w, --gateway-mode      Enable experimental Internet gateway "
        "support.\n"
...
struct sockaddr_in *get_if_info(char *ifname, int type)
{
    ...
    if (ioctl(skfd, type, &ifr) < 0) {
        alog(LOG_ERR, errno, __FUNCTION__, "Could not get address of %s ",
             ifname);
        close(skfd);
        return NULL;
    } else {
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        case 'R':
            ...
            break;
        case 'V':
            printf("\nAODV-UU v%s, %s © Uppsala University & Ericsson "
                   "AB.\nAuthor: Erik Nordström, <erik.nordstrom@it.uu.se>\n\n",
    ...
    while (1) {
        ...
        if ((n = pselect(nfds, &rfds, NULL, NULL, &timeout_spec, &origmask)) <
            0) {
            if (errno != EINTR)
