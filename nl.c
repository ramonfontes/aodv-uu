...
void nl_init(void)
{
    ...
    if (getsockname(aodvnl.sock, (struct sockaddr *)&aodvnl.local, &addrlen) <
        0) {
...
static void nl_kaodv_callback(int sock)
{
    ...
    case NLMSG_ERROR:
        ...
        if (nlmerr->error == 0) {
            ...
            DEBUG(LOG_DEBUG, 0, "NLMSG_ERROR, error=%d type=%s", nlmerr->error,
    ...
    case KAODVM_TIMEOUT:
        ...
        if (rt && rt->state == VALID)
            ...
            DEBUG(LOG_DEBUG, 0, "Got rt timeoute event but there is no route");
        break;
    ...
    case KAODVM_ROUTE_UPDATE:
        ...
        //	DEBUG(LOG_DEBUG, 0, "ROute update s=%s d=%s",
        //ip_to_str(src_addr), ip_to_str(dest_addr));
        if (dest_addr.s_addr == AODV_BROADCAST ||
            dest_addr.s_addr == DEV_IFINDEX(m->ifindex).broadcast.s_addr)
            return;
...
#endif
    return nl_kern_route(RTM_NEWROUTE, NLM_F_CREATE, AF_INET, ifindex, &dest,
                         &next_hop, NULL, metric);
}
